		...
		return class Translator extends Plugin {
			...
			microsoftTranslate (data, callback) {
				BDFDB.LibraryRequires.request("https://api.cognitive.microsofttranslator.com/translate", {
					...
					headers: {
						...
						"Ocp-Apim-Subscription-Key": authKeys.microsoft && authKeys.microsoft.key || "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
			...
			deepLTranslate (data, callback) {
				BDFDB.LibraryRequires.request(authKeys.deepl && authKeys.deepl.paid ? "https://api.deepl.com/v2/translate" : "https://api-free.deepl.com/v2/translate", {
					...
					headers: {
						"Content-Type": "application/json",
						"Authorization": `DeepL-Auth-Key ${authKeys.deepl && authKeys.deepl.key || "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx:fx"}`
					},
			...
			yandexTranslate (data, callback) {
				BDFDB.LibraryRequires.request("https://translate.yandex.net/api/v1.5/tr/translate", {
					form: {
						"key": authKeys.yandex && authKeys.yandex.key || "trnsl.1.1.xxxxxxxxxxxxxxxx.xxxxxxxxxxxxxxxx.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
			...
			papagoTranslate (data, callback) {
				const credentials = (authKeys.papago && authKeys.papago.key || "xxxxxxxxxxxxxxxxxxxx xxxxxxxxxx").split(" ");
			...
			baiduTranslate (data, callback) {
				const credentials = (authKeys.baidu && authKeys.baidu.key || "xxxxxxxxxx xxxxxxxxxxxxxxxxxxxx").split(" ");
		...
		stop () {
			...
			if (this.realInstance) {
				try {
					...
					console.error(`[TranslatorWrapper] Error stopping real instance of ${this.name}:`, e);
				}
			}
		}
		
		getSettingsPanel () {
			if (this.realInstance && typeof this.realInstance.getSettingsPanel === "function") {
				try {
					return this.realInstance.getSettingsPanel();
				} catch (e) {
					console.error(`[TranslatorWrapper] Error showing settings panel for ${this.name}:`, e);
				}
			}
			let template = document.createElement("template");
			template.innerHTML = `<div style="color: var(--text-strong); font-size: 16px; font-weight: 300; white-space: pre; line-height: 22px;">The Library Plugin needed for ${this.name} is loading or missing.</div>`;
			return template.content.firstElementChild;
		}
		
		checkAndStart () {
			if (this.realInstance) return;
			if (window.BDFDB_Global && window.BDFDB_Global.loaded && window.BDFDB_Global.started) {
				try {
					const RealClass = createRealClass();
					this.realInstance = new RealClass(this.meta);
					this.realInstance.load();
					if (this.started) {
						this.realInstance.start();
					}
				} catch (e) {
					console.error(`[TranslatorWrapper] Error initializing real class of ${this.name}:`, e);
				}
			} else {
				if (!this.downloadModalShown && !window.BDFDB_Global && !require("fs").existsSync(require("path").join(BdApi.Plugins.folder, "0BDFDB.plugin.js"))) {
					this.downloadModalShown = true;
					window.BDFDB_Global = Object.assign({}, window.BDFDB_Global, {downloadModal: true});
